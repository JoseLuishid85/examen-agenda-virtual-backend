...
const crearCliente = async (req, res) => { 
    ...
    try {
        const [results] = await connection.query("INSERT INTO cliente(nombre,apellido,telefono) VALUES(?, ?, ?)", [
            req.body.nombre,
            req.body.apellido,
            req.body.telefono,
        ]);
...
const modificarCliente = async (req, res) => {
    ...
    try {
        const [rows] = await connection.query('UPDATE cliente SET nombre = ?, apellido= ?,  telefono = ? WHERE id = ?', [
            nombre,
            apellido,
            telefono,
            id
